...
운영 버전: `2026-08-19-taxonomy-a-f-human-review-v2`
...
5. JSON 강제 파싱
6. 문항별 QA 검증
7. 정답 길이·정답 위치·선택지 표현 패턴 검증
8. 중복 검사
9. `difficulty_metrics` 검증
10. 관리자 미리보기 및 수정
11. 사람 검수 체크리스트 통과
12. 개별 또는 일괄 승인 후 `question_bank` 저장

AUTO 난이도는 최근 정답률 데이터가 있으면 적응형 규칙을 적용하고, 데이터가 없으면 과정별 기본값을 사용한다.

## 자동 QA 및 사람 검수

AI가 생성한 문항은 바로 저장하지 않고 `human_review.status=pending` 상태로 미리보기 목록에 들어간다.

서버 자동 QA 차단 기준:

- placeholder, seed, `문항 #`, `보기 1: 올바른 문장 구조` 같은 개발용 문구
- 실제 지시문·지문·해설·정답 또는 평가기준 누락
- 객관식에서 정답이 여러 개로 해석되는 경우
- 정답 선택지만 지나치게 길거나 구체적인 경우
- 한 번에 생성된 객관식 문항들의 정답 위치가 모두 같은 경우
- 기존 문제와 제목·지문·문제 구조가 과도하게 유사한 경우

장문 예시가 곧 정답처럼 작동하면 학습자의 글쓰기 능력보다 분량과 형식 모방이 정답 단서가 된다. 따라서 문제 생성 엔진은 길이 자체가 정답 단서가 되지 않도록 선택지·예시답안·평가기준의 분량을 조절하고, 근거의 관련성·문단 기능·반론 처리·결론의 재구성 여부를 기준으로 변별력을 만든다.

패턴에 의한 답안 추론을 막기 위해 같은 문항 묶음에서 항상 같은 순서로 장점/단점/결론을 요구하지 않는다. 생성 단계에서 정답 위치 반복, 선택지 길이 편향, 반복 표현, 근거 표지의 빈약함을 QA issue로 남기고, 일정 기준 이상이면 `human_review.status=pending` 상태로 사람 검수 큐에 보낸다.
...
- 오답 선택지도 그럴듯함
- 정답 또는 평가기준이 하나로 명확함
- 표현 패턴만으로 답을 추론할 수 없음
- 이론·난이도·학습 도구 목적과 일치함

AI 생성 문항은 위 수기 체크리스트가 모두 통과되어 `human_review.overall_passed=true`가 기록되어야 `question_bank`에 저장된다.
...
9. 객관식 정답은 길이, 위치, 문체, 구체성만으로 추론할 수 없게 한다.
10. 같은 묶음에서 정답 위치와 선택지 문장 길이·어조를 분산한다.
...
- 정답 위치가 반복되지 않게 한다.
- 정답 선택지가 항상 가장 길거나 가장 자세해 보이지 않게 한다.
- 오답도 정답과 비슷한 길이·구체성·문체를 유지한다.
...
6. 정답이 선택지 길이·위치·문체 패턴으로 드러나지 않는가?
7. 해설이 교육 이론을 설명하는가?
8. 최근 문항과 소재·구조·표현이 반복되지 않는가?
9. 사족, 반복, placeholder, 조사 오류가 없는가?
10. 정답 안에 비문이 남아 있지 않은가?
11. 사람 검수 체크리스트가 모두 통과되었는가?
12. DB JSON으로 파싱 가능한가?
