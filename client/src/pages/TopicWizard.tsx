...
const CATEGORIES = ["사회 현상", "환경 문제", "기술 발전", "교육", "문화", "경제", "정치", "과학"];
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 px-4 py-8 sm:px-6 lg:px-8">
      <main className="mx-auto max-w-3xl">
        ...
        <Card className="border-slate-200 bg-white p-5 shadow-sm sm:p-8">
          ...
          {step === 1 && <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3">{CATEGORIES.map((category) => <Button key={category} variant={data.category === category ? "default" : "outline"} onClick={() => update("category", category)} className={data.category === category ? "bg-emerald-700 hover:bg-emerald-800" : "text-slate-700"}>{category}</Button>)}</div>}
          {step === 2 && <div className="mt-6 space-y-2"><label className="text-sm font-semibold text-slate-800">구체적인 주제</label><Input value={data.topic} onChange={(event) => update("topic", event.target.value)} placeholder="예: 학교에서 생성형 AI 사용 기준은 어떻게 마련해야 할까?" className="h-12" /><p className="text-xs text-slate-500">대상·상황·쟁점 중 두 가지 이상이 드러나도록 작성해 보세요.</p></div>}
