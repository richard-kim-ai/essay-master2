...
import { COURSE_MAP, TOOL_TYPE_MAP, buildShortUserPrompt, generateQuestionBankItems, generationRequestSchema, mapGeneratedItemsToQuestionBank, qaQuestionItem, resolveAdaptiveDifficulty } from "./questionGeneration";
...
function content(course: keyof typeof COURSE_MAP, tool: keyof typeof TOOL_TYPE_MAP, overrides: Record<string, unknown> = {}) {
  return {
    ...
    sampleOutput: { title: "학교 휴대전화 규칙", stance: "공동 기준이 필요하다" }, checklistItems: ["완전한 문장인가?", "주장이 분명한가?", "논증 범위가 적절한가?"], passingStandard: "세 항목 중 두 항목 이상을 충족하고 수정 방향을 제시한다.", ...overrides,
  };
}
